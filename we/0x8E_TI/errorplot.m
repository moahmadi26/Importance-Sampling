N = 15;
pmat = load('~/Desktop/0x8E_TI_Figures/WE/p.mat');
p = pmat.p;

tmat = load('~/Desktop/0x8E_TI_Figures/WE/t.mat');
t = tmat.t;

vmat = load('~/Desktop/0x8E_TI_Figures/WE/v.mat');
v = vmat.v;

circuitTrue = 6.29e-4*ones(size(p));
circuit_TI_True = 8.74e-4*ones(size(p));
...
hold on
plot(x(1:2:end), circuit_TI_True(1:2:end), 'lineWidth', 2)
hold on
