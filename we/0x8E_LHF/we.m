                ...
                for iii=1:sz
                    ...
                    if curr_time < tmax
                        ...
                        while (curr_time - bin_obj(ii).traj_list(iii).time) < time_step
                            a = calculatePropensity0x8E_LHF(curr_x);
                            a0 = sum(a);
                            r1 = rand;
                            r2 = rand;
                            tau = -(1.0/a0) * log(r1);
                    ...
                    if curr_x(9) <= 159
                        bin_obj(ii).traj_list(iii).bin = curr_x(9)+1;
                    else 
                        bin_obj(ii).traj_list(iii).bin = 161;
                    end
                end
            end
        end
            ...
            if sz>0
                for iii=1:sz
                    if bin_obj(ii).traj_list(iii).bin ~= ii
                        if bin_obj(ii).traj_list(iii).bin == 161
                            bin_obj(161).traj_list(end+1) = bin_obj(ii).traj_list(iii);
                        else 
...
% succeed/failed and what is the sum of weight for those
    ...
    sz_161 = bin_size(bin_obj(161)); 
    if sz_161>0
        for ii=1:sz_161
            num_succ = num_succ + 1;
            sum_succ_weights = sum_succ_weights + bin_obj(161).traj_list(ii).weight;
            samples(end+1) = bin_obj(161).traj_list(ii).weight;
        end
    end
