N = 15;
pmat = load('~/Desktop/0x8E_LHF_Figures/WE/p.mat');
p = pmat.p;

tmat = load('~/Desktop/0x8E_LHF_Figures/WE/t.mat');
t = tmat.t;

vmat = load('~/Desktop/0x8E_LHF_Figures/WE/v.mat');
v = vmat.v;

circuitTrue = 6.29e-4*ones(size(p));
circuit0x8E_LHF_True = 7.33e-4*ones(size(p));
...
hold on
errorbar(x(1:1:end), p(1:1:end), err(1:1:end), 'o', 'LineWidth', 2)
hold on
plot(x(1:1:end), circuit0x8E_LHF_True(1:1:end), 'lineWidth', 2)
hold on
plot(x(1:1:end), p(1:1:end), ':', 'LineWidth', 1.5, 'Color', [0 0.447 0.7410])
...
figure(2)
hold on
plot(x(1:1:end), t(1:1:end), 'lineWidth', 2)
legend("Time")
set (gcf, 'color', 'w')
legend boxoff 
xlabel("Bin Size", 'fontsize', 20)
